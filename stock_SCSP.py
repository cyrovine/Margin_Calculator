'''
統一賣出 固定買賣權 權利金2個 履約價2個 股價 風險 A B 乘數 口數
賣出call、賣出put ： {MAXIMUM(賣出call之保證金，賣出put之保證金)＋保證金較低方之權利金市值＋混合部位風險保證金(C值)}*口數
雙買雙賣
'''
...
def stock_SCSP_BS(option1, option2, premiums_1, premiums_2, risk_LV_A, risk_LV_B, performance_price_1, performance_price_2, presently_price, contract, position_1, position_2):
    ...
    #call之保證金
    deposit_C=(premiums_1*contract+max(presently_price*contract*risk_LV_A-max((performance_price_1-presently_price)*contract,0),presently_price*contract*risk_LV_B))*position_1
    #put之保證金
    deposit_P=(premiums_1*contract+max(presently_price*contract*risk_LV_A-max((presently_price-performance_price_1)*contract,0),presently_price*contract*risk_LV_B))*position_1
    ...
    if deposit_C > deposit_P:
        ...
        return(deposit,(premiums_1+premiums_2)*contract*position_1)
    else :
        deposit= deposit_P + (premiums_1*contract*position_1)+(deposit_P-deposit_C)
        return(deposit,(premiums_1+premiums_2)*contract*position_1)
